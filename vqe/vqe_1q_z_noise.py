...
# Supports H = Z (Rx ansatz, ground state |1⟩).
...
def sampled_expectation(angle, simulator, n_samples=1000):
    """Manual Z expectation by counting |0⟩ and |1⟩ outcomes."""
...
def plot_landscape(
    ...
    plt.scatter(
        ...
        marker="D",
        s=75,
        label=f"Minimum: θ={optimal_angle:.3f}, ⟨Z⟩={optimal_energy:.3f}",
    )
...
def plot_convergence(history, optimal_energy):
    """Figure 2: ⟨Z⟩ value at each optimizer iteration."""
    ...
    plt.ylabel(r"$\langle Z \rangle$")
    plt.title(
        f"VQE Convergence\nIterations: {len(history)}   Final ⟨Z⟩: {optimal_energy:.6f}"
    )
    plt.legend()
    plt.grid()
    plt.tight_layout()
...
def main():
    ...
    print(f"  Optimal angle  : {optimal_angle:.6f} rad  (expected pi = {np.pi:.6f})")

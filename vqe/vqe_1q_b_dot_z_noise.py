...
# Hamiltonian: H = -B dot Z (spin in a magnetic field along Z).
# Ground state energy: -|B|.
# For B > 0: ground state |0⟩, optimal θ = 0.
# For B < 0: ground state |1⟩, optimal θ = π.
# Two-parameter Rz·Ry ansatz for consistency with previous programs.
...
# =============================================================================
# Hamiltonian parameter — change B to explore different field strengths
# =============================================================================

B = -1.5  # magnetic field strength; can be positive or negative
...
def build_hamiltonian(B):
    """H = -B*Z"""
    return SparsePauliOp.from_list([("Z", -B)])
...
def sampled_expectation(params, simulator, B, n_samples=1000, seed=None):
    """Sampled <H> = -B * <Z> via a single circuit run."""
    theta, phi = params
    ...
    exp_z = (counts.get("0", 0) - counts.get("1", 0)) / n_samples
...
def plot_landscape(history, optimal_params, optimal_energy, ansatz, hamiltonian, B):
    ...
    plt.title(f"VQE Energy Landscape: H = -B·Z,  B = {B}")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig("vqe_expectation.png", dpi=150)


def plot_convergence(history, optimal_energy, B):
    ...
    plt.axhline(
        ...
        color="red",
        linestyle="--",
        label=f"True ground state: -{abs(B):.4f}",
    )
...
def main():
    # Setup
    ansatz = build_ansatz()
    hamiltonian = build_hamiltonian(B)
    ideal_simulator = build_ideal_simulator()
    noisy_simulator = build_noisy_simulator()
    fake_simulator = build_fake_simulator()

    # Analytic optimal angle depends on sign of B
    theta_star = 0.0 if B > 0 else np.pi

    print(f"\nHamiltonian: H = -B·Z,  B = {B}")
    print(f"True ground state energy : -{abs(B):.6f}")
    print(f"Optimal angle (analytic) : θ* = {theta_star:.4f} rad, φ* = 0.0000 rad")
    print(f"Ground state             : {'|0⟩' if B > 0 else '|1⟩'}")
    ...
    print(
        f"  Ideal sampled        : {
            sampled_expectation(params_check, ideal_simulator, B):.6f}"
    )
    print(
        f"  Noisy sampled        : {
            sampled_expectation(params_check, noisy_simulator, B):.6f}"
    )
    print(
        f"  FakeManilaV2         : {
            sampled_expectation(params_check, fake_simulator, B):.6f}"
    )
    ...
    print(
        f"  Optimal θ      : {optimal_params[0]:.6f} rad  (expected {theta_star:.4f})"
    )
    print(f"  Optimal φ      : {optimal_params[1]:.6f} rad  (expected 0.0000)")
    print(f"  Optimal ⟨H⟩    : {optimal_energy:.6f}     (expected -{abs(B):.6f})")
    print(f"  Iterations     : {len(history)}")

    # Plots
    plot_landscape(history, optimal_params, optimal_energy, ansatz, hamiltonian, B)
    plot_convergence(history, optimal_energy, B)
    draw_histogram(
        3,
        [theta_init, phi_init],
        "Initial Guess",

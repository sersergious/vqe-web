...
# Hamiltonian: H = X (Ry ansatz, ground eigenstate |−⟩).
...
def plot_landscape(
    ...
    plt.scatter(
        ...
        marker="D",
        s=75,
        label=f"Minimum: θ={optimal_angle:.3f}, ⟨X⟩={optimal_energy:.3f}",
    )

    plt.xlabel("Angle [radians]")
    plt.ylabel(r"$\langle X \rangle$")
    plt.title("VQE Energy Landscape: Single Ry Ansatz")
    plt.legend()
    plt.grid()
    plt.tight_layout()
...
def plot_convergence(history, optimal_energy):
    """Figure 2: ⟨X⟩ value at each optimizer iteration."""
    ...
    plt.ylabel(r"$\langle X \rangle$")
    plt.title(
        f"VQE Convergence\nIterations: {len(history)}   Final ⟨X⟩: {optimal_energy:.6f}"
    )
    plt.legend()
    plt.grid()
    plt.tight_layout()
...
def build_histogram_data(
    ...
    # H rotates X eigenbasis to Z eigenbasis
    qc = QuantumCircuit(1, 1)
    qc.ry(angle, 0)
    qc.h(0)
    qc.measure(0, 0)

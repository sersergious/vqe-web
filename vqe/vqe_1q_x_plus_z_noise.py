...
# Hamiltonian: H = X + Z (two-parameter Rz·Ry ansatz).
# Ground state energy: -sqrt(2) ≈ -1.4142
...
def build_hamiltonian():
    # H = X + Z
    return SparsePauliOp.from_list([("X", 1.0), ("Z", 1.0)])
...
def plot_landscape(history, optimal_params, optimal_energy, ansatz, hamiltonian):
    """Figure 1: 2D heatmap of <X+Z> over (theta, phi) parameter space
    ...
    # Optimizer path in parameter space
    ...
    plt.colorbar(cp, label=r"$\langle X + Z \rangle$")
    plt.plot(
        path_phi,
        path_theta,
        "w-o",
    ...
    plt.title("VQE Energy Landscape: H = X + Z")
...
def plot_convergence(history, optimal_energy):
    """Figure 2: same as landscape for two-parameter case."""
    ...
    plt.axhline(
        ...
        color="red",
        linestyle="--",
        label=f"True ground state: -√2 ≈ {-np.sqrt(2):.4f}",
    )
    plt.xlabel("Optimizer iteration")
    plt.ylabel(r"$\langle X + Z \rangle$")
    plt.title(
...
def build_histogram_data(
    ...
    # Measurement circuit in Z basis
...
def main():
    ...
    print(f"\nTrue ground state energy: -√2 = {-np.sqrt(2):.6f}")
    ...
    print(
        f"  Ideal sampled        : {
            sampled_expectation(params_check, ideal_simulator):.6f}"
    )
    print(
        f"  Noisy sampled        : {
            sampled_expectation(params_check, noisy_simulator):.6f}"
    )
    print(
        f"  FakeManilaV2         : {
            sampled_expectation(params_check, fake_simulator):.6f}"
    )
    ...
    print(
        f"  Optimal ⟨H⟩    : {optimal_energy:.6f}     (expected -√2 = {
            -np.sqrt(2):.6f})"
    )
    print(f"  Iterations     : {len(history)}")

    # Plots
    plot_landscape(history, optimal_params, optimal_energy, ansatz, hamiltonian)
